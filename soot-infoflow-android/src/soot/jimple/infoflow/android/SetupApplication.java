...
public class SetupApplication {
	...
	protected List<ARSCFileParser.ResPackage> resourcePackages = null;
	protected String appPackageName = "";

	protected final String androidJar;
	protected final boolean forceAndroidJar;
	protected final String apkFileLocation;
	private final String additionalClasspath;
	private ITaintPropagationWrapper taintWrapper;

	protected AccessPathBasedSourceSinkManager sourceSinkManager = null;
	protected AndroidEntryPointCreator entryPointCreator = null;

	public IInfoflowConfig sootConfig = new SootConfigForAndroid();
	 ...
	 */
	public void calculateSourcesSinksEntrypoints(Set<AndroidMethod> sources, Set<AndroidMethod> sinks)
...
//			parser.getSinks();
	...
	public void calculateSourcesSinksEntrypoints(ISourceSinkDefinitionProvider sourcesAndSinks)
		...
		this.entrypoints = processMan.getEntryPointClasses();//Gets all classes the contain entry points in this applications四大组件

		// Parse the resource file
		long beforeARSC = System.nanoTime();
		ARSCFileParser resParser = new ARSCFileParser();//Parser for reading out the contents of Android's resource.arsc file. Structure declarations and comments taken from the Android source code and ported from C to Java.
		resParser.parse(apkFileLocation);
		logger.info("ARSC file parsing took " + (System.nanoTime() - beforeARSC) / 1E9 + " seconds");
		this.resourcePackages = resParser.getPackages();// Its value is [soot.jimple.infoflow.android.resources.ARSCFileParser$ResPackage@3e6fa38a]
		...
		if (config.getEnableCallbacks()) {//Gets whether the taint analysis shall consider callbacks
			if (callbackClasses != null && callbackClasses.isEmpty()) {
				...
				switch (config.getCallbackAnalyzer()) {//Gets the callback analyzer that is being used in preparation for the taint analysis
				case Fast:
		...
		// Create the SourceSinkManager
		{
			...
			sourceSinkManager = new AccessPathBasedSourceSinkManager(this.sourceSinkProvider.getSources(),
					...
					lfp == null ? null : lfp.getUserControlsByID());//SourceSinkManager for Android applications. This class uses precise access path-based source and sink definitions.
	...
	private void calculateCallbackMethods(ARSCFileParser resParser, LayoutFileParser lfp) throws IOException {
		...
		while (hasChanged) {
			...
			PackManager.v().getPack("wjpp").apply();//???wjpp
	 ...
	 *            and components
	 */
	protected void collectXmlBasedCallbackMethods(ARSCFileParser resParser, LayoutFileParser lfp,
						...
						for (String methodName : callbackMethods) {
							...
							while (true) {
								...
								if (callbackMethod != null) {
									...
									System.out.println("*****************xml based callback*************************");
									System.out.println(callbackMethod.getBytecodeSignature());
									System.out.println("*****************xml based callback*************************");
									break;
								}
		...
		// Add the callback methods as sources and sinks
		{
			...
			System.out.println("Found " + callbacksPlain.size() + " callback methods for " + this.callbackMethods.size()
					+ " components");
		}
	}
	 ...
	 *             Thrown if a required configuration cannot be read
	 */
	protected void calculateCallbackMethodsFast(ARSCFileParser resParser, LayoutFileParser lfp) throws IOException {
	 ...
	 */
	protected void createMainMethod() {
	 ...
	 *            The callback to be invoked when data flow results are
	 *            available
	 * @return The results of the data flow analysis
	 */
	public InfoflowResults runInfoflow(ResultsAvailableHandler onResultsAvailable) {
		...
		Infoflow info;
		if (cfgFactory == null)//Interface for all factories that can create bi-directional interprocedural control flow graphs.
			//forceAndroidJar false代表传入的是一个android jar的目录 ，true代表传入的是一个android-jar的文件
			{info = new Infoflow(androidJar, forceAndroidJar, null,//main infoflow class which triggers the analysis and offers method to customize it.
					new DefaultPathBuilderFactory(config.getPathBuilder(), config.getComputeResultPaths()));
					/*
					    builder The path building algorithm to use 第一个参数，是什么敏感的,比如流敏感
					 	reconstructPaths Specifies whether the （exact propagation paths） 
					 */
		...
		if (forceAndroidJar)
			...
			path = Scene.v().getAndroidJarPath(androidJar, apkFileLocation);//Scene:Manages the SootClasses of the application being analyzed.
		//

		info.setTaintWrapper(taintWrapper);//设置数据流分析的污点
		//Sets the taint wrapper for deciding on taint propagation through black-box methods
		if (onResultsAvailable != null)
			info.addResultsAvailableHandler(onResultsAvailable);//设置分析完之后的回调

		System.out.println("Starting infoflow computation...");
		info.setConfig(config);
		info.setSootConfig(sootConfig);

		if (null != ipcManager) {//The IPC manager to use for modelling inter-component and inter-application data flows
			info.setIPCManager(ipcManager);//这里ipcManager怎么构造 iccta干的事情！！！
		}

		info.computeInfoflow(apkFileLocation, path, entryPointCreator, sourceSinkManager);
		////SourceSinkManager for Android applications. This class uses precise access path-based source and sink definitions.
		//entryPointCreator 入口点设置
	...
	protected AndroidEntryPointCreator createEntryPointCreator() {
		AndroidEntryPointCreator entryPointCreator = new AndroidEntryPointCreator(//this.entrypoints就是APP的四大组件还有application标签
				new ArrayList<String>(this.entrypoints)); //class which creates a dummy main method with the entry points according to the Android lifecycles based on: http://developer.android.com/reference/android/app/Activity.html#ActivityLifecycle and http://developer.android.com/reference/android/app/Service.html and http://developer.android.com/reference/android/content/BroadcastReceiver.html#ReceiverLifecycle and http://developer.android.com/reference/android/content/BroadcastReceiver.html
		Map<String, List<String>> callbackMethodSigs = new HashMap<String, List<String>>();//dummyMainMethod不存在clinit？
